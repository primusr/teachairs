...
# 1. Fetch your key from Colab secrets
my_api_key = userdata.get('AIzaSyB-5YS3-Mlj3-AajOj9PwCt4yqRdoTnBHU')
genai.configure(api_key=my_api_key)

# 2. Initialize the model using the original variable name
gemini_model = genai.GenerativeModel('gemini-2.5-flash')

# 3. Make your request using the new client format
response = gemini_model.generate_content(
    contents='Analyze this student feedback...'
)
...
CONFIG = {
    "input_csv_path": "/content/2nd Tri - test.csv", #Change folder name if must
    ...
    "lda_stopwords": ['mam', 'maam', 'sir', 'po', 'lang', 'naman', 'wala', 'nya', 'sana','da','en', 'mag', 'pala','kasi', 'wag', 'tsaka', 'di', 'pang', 'pag', 'thankyou',  'po', 'ako', 'naman', 'kita','ur','jan','kay','niyo','rin','paki','ta','ata','kayo'],
    "find_optimal_lda_topics": True,
    "lda_topic_range_start": 3,
    "lda_topic_range_end": 5,  # REDUCED: Only test 3, 4, 5 topics (saves API calls)
    "lda_topic_range_step": 1,
    "lda_default_num_topics": 4,  # REDUCED: Default to 4 topics
    "lda_passes": 25,
    "lda_random_state": 42,
    "gemini_model_name": 'gemini-2.5-flash', # Upgraded to Gemini Pro
    "top_n_words_for_topic_labeling": 8,  # REDUCED: Fewer words = shorter prompts
    "top_n_words_for_topic_gemini_summary": 5,  # REDUCED
    "num_topics_for_overall_gemini_summary": 2,  # REDUCED: Only top 2 topics
    "num_ai_recs_per_topic_subset": 2,  # REDUCED: Only 2 detailed topic recs
}
...
# Initialize stopwords and preprocessing tools
iso_stopwords = custom_iso_stopwords(["en", "tl"])
...
def load_filipino_vader_lexicon(csv_path):
    try:
        if not os.path.exists(csv_path):
            print(f"Warning: Lexicon file '{csv_path}' not found. Attempting upload...")
            uploaded_lex_files = files.upload()
            if os.path.basename(csv_path) not in uploaded_lex_files:
                print(f"Error: File not uploaded.")
                return {}
        ...
        if 'Word' not in lex_df.columns or 'Sentiment_Score' not in lex_df.columns:
            print(f"Warning: CSV must have 'Word' and 'Sentiment_Score' columns.")
            return {}
        filipino_lexicon = {
            str(row['Word']).lower(): float(row['Sentiment_Score'])
            for index, row in lex_df.iterrows()
            if pd.notnull(row['Word']) and pd.notnull(row['Sentiment_Score'])
        }
        print(f"Loaded {len(filipino_lexicon)} Filipino words.")
...
if filipino_vader_words:
    ...
    print("VADER lexicon augmented.")
else:
    print("VADER lexicon not augmented.")

# ============================================================================
# MAXIMUM SAFETY RATE LIMITER
# ============================================================================
class APIRateLimiter:
    #Ultra-conservative rate limiter - 5 requests per minute maximum.#
    ...
    def __init__(self):
        ...
        self.RPM_LIMIT = 5  # Only 5 requests per minute (super safe)
        self.MIN_DELAY = 12  # Minimum 12 seconds between requests
    ...
    def wait_if_needed(self):
        ...
        avg_rpm = self.total_requests / elapsed_min if elapsed_min > 0 else 0
        print(f"📊 {requests_in_window} req in 60s | Total: {self.total_requests} | Avg: {avg_rpm:.1f} RPM | Failed: {self.failed_requests}")

        # Check if we need to wait due to rate limit
        if requests_in_window >= self.RPM_LIMIT:
            oldest = self.request_times[0]
            wait_time = 60 - (current_time - oldest) + 2
            if wait_time > 0:
        ...
        if self.request_times:
            ...
            if time_since_last < self.MIN_DELAY:
                ...
                print(f"⏰ Min delay: waiting {wait_time:.1f}s...")
                time.sleep(wait_time)
                current_time = time.time()
...
gemini_model = None
try:
    GEMINI_API_KEY = userdata.get('AIzaSyB-5YS3-Mlj3-AajOj9PwCt4yqRdoTnBHU')
    ...
    print(f"📊 Rate limit: {rate_limiter.RPM_LIMIT} RPM (minimum {rate_limiter.MIN_DELAY}s between calls)")

except Exception as e:
    print(f"❌ API error: {e}")
    gemini_model = None

def make_gemini_request_with_retry(prompt_text, max_retries=10):
    #Make API request with maximum safety and patience.#
    if not gemini_model:
        return "Gemini not configured."

    print(f"\n🔄 API request (up to {max_retries} retries)...")
    ...
    for attempt in range(max_retries):
        try:
            print(f"   Try {attempt + 1}/{max_retries}...", end=' ')
            ...
            if "429" in error_str or "rate limit" in error_str or "too many" in error_str:
                ...
                rate_limiter.record_failure()
                return f"Quota exceeded - wait until tomorrow or upgrade to paid tier"
...
print("="*70)
print(f"Rate limit: {rate_limiter.RPM_LIMIT} requests per minute")
print(f"Min delay: {rate_limiter.MIN_DELAY} seconds between requests")
print(f"This is VERY slow but will not hit rate limits")
print("="*70)
...
def clean_dataframe(df, preferred_text_column_names):
    ...
    if not original_text_column:
        if df.shape[1] > 0 and (df.columns[0] == 0 or isinstance(df.columns[0], int)):
            ...
            if original_text_column:
                print(f"Warning: Preferred cols not found. Guessed: '{original_text_column}'")
...
if not os.path.exists(csv_file_path):
    ...
    try:
        ...
        cleaned_df = clean_dataframe(df_raw, CONFIG["default_feedback_column_names"])
        cleaned_df['Cleaned_Text_Main'] = cleaned_df['Feedback_Text'].apply(lambda x: preprocess_text(x, for_lda=True))
        cleaned_df = cleaned_df[cleaned_df['Cleaned_Text_Main'] != ''].copy()
...
if lda_model and not cleaned_df.empty and 'Cleaned_Text_Main' in cleaned_df:
    ...
    if 'corpus' not in locals() or not corpus: print("Error: LDA corpus not found.")
        ...
        #This sections is for the Vertical Display
        print("\nOverall Sentiment Per Topic:")
        if not df_topic_sentiments.empty:
            for index, row in df_topic_sentiments.iterrows():
                # Clean up the AI label for better display
                ai_label = str(row.get('AI Label', 'N/A')).replace('\n', ' | ')

                print(f"\n--- Topic {row.get('Topic ID', 'N/A')}: {ai_label} ---")
                print(f"  - Top Keywords: {row.get('Top Keywords', 'N/A')}")
                print(f"  - Comment Count: {row.get('Num Comments', 'N/A')}")

                if 'Avg VADER Aug Score' in row and pd.notna(row['Avg VADER Aug Score']):
                     print(f"  - Augmented VADER Score (Avg): {row.get('Avg VADER Aug Score', 'N/A')}")
                     dist_aug = row.get('VADER Aug Dist (%)', {})
                     if dist_aug:
                         dist_str_aug = ', '.join([f"{k.replace(' (VADER Aug)', '')}: {v}%" for k, v in dist_aug.items()])
                         print(f"    Distribution: {dist_str_aug}")

                if 'Fil. Keyword Score' in row and pd.notna(row['Fil. Keyword Score']):
                    print(f"  - Filipino Keyword Score (Avg): {row.get('Avg Fil. Keyword Score', 'N/A')}")
                    dist_fil = row.get('Fil. Keyword Dist (%)', {})
                    if dist_fil:
                         dist_str_fil = ', '.join([f"{k.replace(' (Filipino Keywords)', '')}: {v}%" for k, v in dist_fil.items()])
                         print(f"    Distribution: {dist_str_fil}")
            print("-" * 50)
            ...
            print("No topic sentiment data to display.")
...
if gemini_model and lda_model and hasattr(lda_model, 'num_topics') and lda_model.num_topics > 0:
    ...
    if 'df_topic_sentiments' in locals() and not df_topic_sentiments.empty and 'Avg VADER Aug Score' in df_topic_sentiments.columns:
        df_topic_sentiments['Avg VADER Aug Score Numeric'] = pd.to_numeric(df_topic_sentiments['Avg VADER Aug Score'], errors='coerce')
        df_topic_sentiments_recs = df_topic_sentiments.dropna(subset=['Avg VADER Aug Score Numeric']).copy()
        ...
        if not df_topic_sentiments_recs.empty:
            ...
            print(f"Generating recommendations for the {len(topic_ids_for_recs)} topic(s) with the most negative/neutral sentiment.")
        else:
            print("No topics with valid sentiment scores for ranking. Defaulting to first N topics.")
            topic_ids_for_recs = list(range(min(num_topics_for_ai_recs, lda_model.num_topics)))
    else:
        print(f"Warning: Topic sentiment data not available. Defaulting to first {num_topics_for_ai_recs} topics.")
        topic_ids_for_recs = list(range(min(num_topics_for_ai_recs, lda_model.num_topics)))
    ...
    for topic_id in topic_ids_for_recs:
        ...
        print(f"\n------------------------------------------------------------")
        print(f"RECOMMENDATIONS FOR TOPIC {topic_id}: {ai_generated_label}")
        print(f"------------------------------------------------------------")
        ...
        if "Error" in recommendation_text_raw or "Failed" in recommendation_text_raw or "stopped" in recommendation_text_raw:
            recommendation_text = f"Recommendation generation failed: {recommendation_text_raw}"
            rec_type += ' - ERROR'
        else:
            recommendation_text = recommendation_text_raw.strip()

        display_wrapped_text(recommendation_text)
...
#8.1.Sentiment Analysis (Tabular View)#

print("\n" + "="*70)
print("📊 SENTIMENT ANALYSIS SUMMARY (Tabular View)")
print("="*70)

if not cleaned_df.empty:
    # Display first 10 rows in a clean table
    display_cols = ['Original_Text', 'VADER_Sentiment_Eng', 'VADER_Score_Eng',
                    'VADER_Sentiment_Aug', 'VADER_Score_Aug',
                    'Filipino_Keyword_Sentiment', 'Filipino_Keyword_Score']

    available_cols = [col for col in display_cols if col in cleaned_df.columns]

    print("\n📋 First 10 Feedback Results:")
    display(cleaned_df[available_cols].head(10))
    ...
    if 'VADER_Sentiment_Eng' in cleaned_df.columns:
        vader_eng_counts = cleaned_df['VADER_Sentiment_Eng'].value_counts()
        summary_data.append({
            'Analysis Method': 'VADER (English)',
            'Positive': vader_eng_counts.get('Positive (VADER Eng)', 0),
            'Neutral': vader_eng_counts.get('Neutral (VADER Eng)', 0),
            'Negative': vader_eng_counts.get('Negative (VADER Eng)', 0),
            'Total': len(cleaned_df)
        })

    if 'VADER_Sentiment_Aug' in cleaned_df.columns:
        vader_aug_counts = cleaned_df['VADER_Sentiment_Aug'].value_counts()
        summary_data.append({
            'Analysis Method': 'VADER (Augmented)',
            'Positive': vader_aug_counts.get('Positive (VADER Aug)', 0),
            'Neutral': vader_aug_counts.get('Neutral (VADER Aug)', 0),
            'Negative': vader_aug_counts.get('Negative (VADER Aug)', 0),
            'Total': len(cleaned_df)
        })

    if 'Filipino_Keyword_Sentiment' in cleaned_df.columns:
        fil_counts = cleaned_df['Filipino_Keyword_Sentiment'].value_counts()
        summary_data.append({
            'Analysis Method': 'Filipino Keywords',
            'Positive': fil_counts.get('Positive (Filipino Keywords)', 0),
            'Neutral': fil_counts.get('Neutral (Filipino Keywords)', 0),
            'Negative': fil_counts.get('Negative (Filipino Keywords)', 0),
            'Total': len(cleaned_df)
        })
    ...
    if 'VADER_Score_Eng' in cleaned_df.columns:
        scores_data.append({
            'Analysis Method': 'VADER (English)',
            'Mean Score': cleaned_df['VADER_Score_Eng'].mean(),
            'Median Score': cleaned_df['VADER_Score_Eng'].median(),
            'Std Dev': cleaned_df['VADER_Score_Eng'].std()
        })

    if 'VADER_Score_Aug' in cleaned_df.columns:
        scores_data.append({
            'Analysis Method': 'VADER (Augmented)',
            'Mean Score': cleaned_df['VADER_Score_Aug'].mean(),
            'Median Score': cleaned_df['VADER_Score_Aug'].median(),
            'Std Dev': cleaned_df['VADER_Score_Aug'].std()
        })

    if 'Filipino_Keyword_Score' in cleaned_df.columns:
        scores_data.append({
            'Analysis Method': 'Filipino Keywords',
            'Mean Score': cleaned_df['Filipino_Keyword_Score'].mean(),
            'Median Score': cleaned_df['Filipino_Keyword_Score'].median(),
            'Std Dev': cleaned_df['Filipino_Keyword_Score'].std()
        })

    scores_summary_df = pd.DataFrame(scores_data)
    scores_summary_df = scores_summary_df.round(4)
    display(scores_summary_df)
...
#8.2. Scatter Plots of VADER Polarity Scores#
...
if gemini_model:
    ...
    if not cleaned_df.empty:
        ...
        if lda_model and hasattr(lda_model, 'num_topics') and lda_model.num_topics > 0:
            ...
            for i_topic in range(num_topics_to_include):
                ...
                ai_lbl_sum = topic_labels_by_ai.get(i_topic, "Label N/A")
        ...
        display_wrapped_text(overall_recommendation_text)
...
if all_gemini_recommendations_data:
    recommendations_df = pd.DataFrame(all_gemini_recommendations_data)

    print(f"\n✅ Generated {len(recommendations_df)} AI recommendations")
    print(f"\n📋 Recommendation Types:")
    display(recommendations_df['Type'].value_counts().to_frame('Count'))
    ...
    for idx, row in recommendations_df.head(3).iterrows():
        ...
        print(f"\nInput Summary:")
        print(row['Input_Summary_To_Gemini'][:200] + "..." if len(row['Input_Summary_To_Gemini']) > 200 else row['Input_Summary_To_Gemini'])
        print(f"\nRecommendation Preview:")
        print(row['Gemini_Recommendations'][:300] + "..." if len(row['Gemini_Recommendations']) > 300 else row['Gemini_Recommendations'])
else:
...
def manual_input_interface():
    ...
    while True:
        ...
        display_wrapped_text(manual_recommendation_text_raw) # <-- UPDATED LINE
