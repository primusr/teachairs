            ...
            for i in range(0, len(topic_ids_to_plot), 2):
                ...
                for j, col in enumerate(cols):
                    ...
                    with col:
                        ...
                        buf.seek(0)
                        pre_generated_wordclouds[topic_idx] = f"data:image/png;base64,{base64.b64encode(buf.read()).decode()}"
                        plt.close(fig)
    else:
...
{rec_3}
   ...
   * **Measurement:** Compare learner feedback and engagement before and after implementing the changes."""
    ...
    if gemini_model:
        ...
        # ==========================================
        # ZIP REPORT GENERATOR - REUSING MEMORY ARTIFACTS
        # ==========================================
        st.divider()
        ...
        <img src="{fig_to_base64(fig1)}" class="pdf-img"/>
        
        <h2>Standard VADER Polarity</h2>
        <img src="{fig_to_base64(fig_std)}" class="pdf-img"/>
        
        <h2>Augmented VADER Polarity</h2>
        <img src="{fig_to_base64(fig_aug)}" class="pdf-img"/>
        ...
        # 2. Append Cached Word Clouds directly
        if pre_generated_wordclouds:
            full_sections_html += "<div class='page-break'></div><h2>Topic Word Clouds</h2>"
            for topic_idx, wc_b64 in pre_generated_wordclouds.items():
                ai_title = next((r["AI Label"] for r in topic_rows if r["Topic ID"] == topic_idx), f"Topic {topic_idx}")
                full_sections_html += f"""
        ...
        # 3. Append Cached Topic Recommendations directly
        if pre_generated_topic_recs:
            full_sections_html += "<div class='page-break'></div><h2>AI Recommendations for Selected Topics</h2>"
            for rec in pre_generated_topic_recs:
                formatted_rec = rec["text"].replace("\n", "<br>").replace("**", "<b>")
                full_sections_html += f"""
                <div class="recommendation-card">
                    <h3>Topic {rec['id']}: {rec['label']}</h3>
                    <div class="text-block">{formatted_rec}</div>
                </div>
                """

        # 4. Append Cached Executive Summary directly
        if gemini_recommendation_text:
        ...
        # 5. Core Responsive Stylesheets for Letter Portrait limits
        html_content = f"""
        <!DOCTYPE html>
        <html>
        <head>
        ...
        h1 {{ font-size: 22pt; border-bottom: 3px solid #3b82f6; padding-bottom: 8px; color: #1e3a8a; }}
        h2 {{ font-size: 14pt; margin-top: 25px; border-bottom: 1px solid #e5e7eb; padding-bottom: 4px; color: #1e40af; page-break-after: avoid; }}
        h3 {{ font-size: 11pt; margin-top: 15px; color: #374151; }}
        .metric-box {{ background: #f3f4f6; border-left: 5px solid #3b82f6; padding: 12px; margin-bottom: 20px; border-radius: 4px; }}
        .pdf-img {{ width: 100%; max-width: 100%; height: auto; display: block; margin: 10px 0; page-break-inside: avoid; }}
        
        /* Rigid page-width locking rules for database columns */
        .report-table {{ width: 100%; border-collapse: collapse; table-layout: fixed; font-size: 7.5pt; margin-top: 10px; }}
        .report-table th, .report-table td {{ border-bottom: 1px solid #e5e7eb; padding: 6px 4px; word-wrap: break-word; overflow-wrap: anywhere; white-space: normal; vertical-align: top; }}
        .report-table th {{ background: #1e3a8a; color: white; text-align: left; }}
        
        .recommendation-card {{ background: #ffffff; border: 1px solid #e5e7eb; border-radius: 6px; padding: 12px; margin-bottom: 15px; page-break-inside: avoid; }}
        .executive-box {{ background: #f8fafc; border-left: 5px solid #10b981; padding: 15px; margin-top: 10px; border-radius: 4px; }}
        .text-block {{ font-size: 10pt; color: #334155; line-height: 1.6; }}
        ...
        # Compile PDF and assembly deployment packages
