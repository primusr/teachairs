...
def build_simple_pdf(report_text, file_name="TechAireReport.pdf"):
    """Create a simple PDF byte stream from plain text content."""
    ...
    current_chunk = []
    for line in lines:
        if len(current_chunk) >= max_lines_per_page:
    ...
    for chunk in page_chunks:
        ...
        for idx, line in enumerate(chunk):
            y = page_height - 72 - (idx * line_height)
        ...
        objects.append((content_obj_id, f"{content_obj_id} 0 obj\n<< /Length {content_obj_id + 1} 0 R >>\nstream\n{content}\nendstream\nendobj"))
        length_obj_id = content_obj_id + 1
        objects.append((length_obj_id, f"{length_obj_id} 0 obj\n{len(content.encode('latin-1', 'replace'))}\nendobj"))

        page_obj_id = len(objects) + 3
        objects.append((page_obj_id, f"{page_obj_id} 0 obj\n<< /Type /Page /Parent 2 0 R /MediaBox [0 0 {page_width} {page_height}] /Contents {content_obj_id} 0 R /Resources << /Font << /F1 1 0 R >> >> >>\nendobj"))
        page_refs.append(page_obj_id)

    page_count = len(page_chunks)
    page_tree_obj = f"2 0 obj\n<< /Type /Pages /Kids [{' '.join(str(ref) + ' 0 R' for ref in page_refs)}] /Count {page_count} >>\nendobj"
    font_obj = "1 0 obj\n<< /Type /Font /Subtype /Type1 /BaseFont /Helvetica >>\nendobj"
    catalog_obj = "3 0 obj\n<< /Type /Catalog /Pages 2 0 R >>\nendobj"

    pdf = ["%PDF-1.4", "1 0 obj\n<< /Type /Catalog /Pages 2 0 R >>\nendobj"]
    pdf.append(page_tree_obj)
    pdf.append(font_obj)
    for _, obj in objects:
        pdf.append(obj)
    pdf.append(catalog_obj)
